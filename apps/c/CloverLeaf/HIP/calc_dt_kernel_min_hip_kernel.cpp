...
//
__constant__ int dims_calc_dt_kernel_min [2][1];
static int dims_calc_dt_kernel_min_h [2][1] = {{0}};
...
int size1 ){
  ...
  if (size0==-1) dims_calc_dt_kernel_min[0][0]=0;
  ...
  arg0 += idx_x * 1*1 + idx_y * 1*1 * dims_calc_dt_kernel_min[0][0];

  if (idx_x < size0 && idx_y < size1) {
    const ACC<double> argp0(dims_calc_dt_kernel_min[0][0], arg0);
    calc_dt_kernel_min_gpu(
     argp0, arg1_l);
  }
  for (int d=0; d<1; d++)
    ops_reduction_hip<OPS_MIN>(&arg1[d+(hipBlockIdx_x+ hipBlockIdx_y*hipGridDim_x)*1],arg1_l[d]);
...
void ops_par_loop_calc_dt_kernel_min_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_calc_dt_kernel_min_h[0][0]) {
    dims_calc_dt_kernel_min_h[0][0] = xdim0;
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_calc_dt_kernel_min), dims_calc_dt_kernel_min_h, sizeof(dims_calc_dt_kernel_min)));
  }
  ...
  for (int d=0; d<arg1.dim; d++) ((double *)arg1.data)[d+b*arg1.dim] = INFINITY_double;
  reduct_bytes += ROUND_UP(maxblocks*arg1.dim*sizeof(double));
  ...
  if (x_size > 0 && y_size > 0)
    hipLaunchKernelGGL(ops_calc_dt_kernel_min, grid, tblock, nshared, 0,
