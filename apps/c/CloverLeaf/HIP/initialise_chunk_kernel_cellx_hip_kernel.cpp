...
int size1 ){
  ...
  if (size0==-1) dims_initialise_chunk_kernel_cellx[0][0]=0;


  int idx_y = hipBlockDim_y * hipBlockIdx_y + hipThreadIdx_y;
  int idx_x = hipBlockDim_x * hipBlockIdx_x + hipThreadIdx_x;

  arg0 += idx_x * 1*1 + idx_y * 0*1 * dims_initialise_chunk_kernel_cellx[0][0];
  arg1 += idx_x * 1*1 + idx_y * 0*1 * dims_initialise_chunk_kernel_cellx[1][0];
  ...
  if (idx_x < size0 && idx_y < size1) {
    ...
    initialise_chunk_kernel_cellx_gpu(
     argp0, argp1, argp2);
  }
...
void ops_par_loop_initialise_chunk_kernel_cellx_execute(ops_kernel_descriptor *desc) {
  ...
  if (block->instance->OPS_diags > 1) {
    ops_timing_realloc(block->instance,4,"initialise_chunk_kernel_cellx");
  ...
  if (xdim0 != dims_initialise_chunk_kernel_cellx_h[0][0] || xdim1 != dims_initialise_chunk_kernel_cellx_h[1][0] || xdim2 != dims_initialise_chunk_kernel_cellx_h[2][0]) {
    ...
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_initialise_chunk_kernel_cellx), dims_initialise_chunk_kernel_cellx_h, sizeof(dims_initialise_chunk_kernel_cellx)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    hipLaunchKernelGGL(ops_initialise_chunk_kernel_cellx, grid, tblock, 0, 0,
