...
int size1 ){
  ...
  if (size0==-1) dims_calc_dt_kernel_get[0][0]=0;
  ...
  if (idx_x < size0 && idx_y < size1) {
    const ACC<double> argp0(dims_calc_dt_kernel_get[0][0], arg0);
    const ACC<double> argp1(dims_calc_dt_kernel_get[1][0], arg1);
    calc_dt_kernel_get_gpu(
     argp0, argp1, arg2_l, arg3_l);
  }
  for (int d=0; d<1; d++)
    ops_reduction_hip<OPS_INC>(&arg2[d+(hipBlockIdx_x+ hipBlockIdx_y*hipGridDim_x)*1],arg2_l[d]);
  for (int d=0; d<1; d++)
    ops_reduction_hip<OPS_INC>(&arg3[d+(hipBlockIdx_x+ hipBlockIdx_y*hipGridDim_x)*1],arg3_l[d]);
...
void ops_par_loop_calc_dt_kernel_get_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_calc_dt_kernel_get_h[0][0] || xdim1 != dims_calc_dt_kernel_get_h[1][0]) {
    dims_calc_dt_kernel_get_h[0][0] = xdim0;
    dims_calc_dt_kernel_get_h[1][0] = xdim1;
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_calc_dt_kernel_get), dims_calc_dt_kernel_get_h, sizeof(dims_calc_dt_kernel_get)));
  }
  ...
  for (int d=0; d<arg2.dim; d++) ((double *)arg2.data)[d+b*arg2.dim] = ZERO_double;
  reduct_bytes += ROUND_UP(maxblocks*arg2.dim*sizeof(double));

  arg3.data = block->instance->OPS_reduct_h + reduct_bytes;
  arg3.data_d = block->instance->OPS_reduct_d + reduct_bytes;
  for (int b=0; b<maxblocks; b++)
  for (int d=0; d<arg3.dim; d++) ((double *)arg3.data)[d+b*arg3.dim] = ZERO_double;
  reduct_bytes += ROUND_UP(maxblocks*arg3.dim*sizeof(double));
  ...
  if (x_size > 0 && y_size > 0)
    hipLaunchKernelGGL(ops_calc_dt_kernel_get, grid, tblock, nshared, 0,
