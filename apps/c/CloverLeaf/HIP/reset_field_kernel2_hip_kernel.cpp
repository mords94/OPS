...
//
__constant__ int dims_reset_field_kernel2 [4][1];
static int dims_reset_field_kernel2_h [4][1] = {{0}};
...
int size1 ){
  ...
  if (size0==-1) dims_reset_field_kernel2[0][0]=0;


  int idx_y = hipBlockDim_y * hipBlockIdx_y + hipThreadIdx_y;
  int idx_x = hipBlockDim_x * hipBlockIdx_x + hipThreadIdx_x;

  arg0 += idx_x * 1*1 + idx_y * 1*1 * dims_reset_field_kernel2[0][0];
  arg1 += idx_x * 1*1 + idx_y * 1*1 * dims_reset_field_kernel2[1][0];
  arg2 += idx_x * 1*1 + idx_y * 1*1 * dims_reset_field_kernel2[2][0];
  arg3 += idx_x * 1*1 + idx_y * 1*1 * dims_reset_field_kernel2[3][0];

  if (idx_x < size0 && idx_y < size1) {
    ACC<double> argp0(dims_reset_field_kernel2[0][0], arg0);
    const ACC<double> argp1(dims_reset_field_kernel2[1][0], arg1);
    ACC<double> argp2(dims_reset_field_kernel2[2][0], arg2);
    const ACC<double> argp3(dims_reset_field_kernel2[3][0], arg3);
    reset_field_kernel2_gpu(
...
void ops_par_loop_reset_field_kernel2_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_reset_field_kernel2_h[0][0] || xdim1 != dims_reset_field_kernel2_h[1][0] || xdim2 != dims_reset_field_kernel2_h[2][0] || xdim3 != dims_reset_field_kernel2_h[3][0]) {
    ...
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_reset_field_kernel2), dims_reset_field_kernel2_h, sizeof(dims_reset_field_kernel2)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    hipLaunchKernelGGL(ops_reset_field_kernel2, grid, tblock, 0, 0,
