...
int size1 ){
  ...
  if (size0==-1) dims_flux_calc_kernely[0][0]=0;


  int idx_y = hipBlockDim_y * hipBlockIdx_y + hipThreadIdx_y;
  int idx_x = hipBlockDim_x * hipBlockIdx_x + hipThreadIdx_x;

  arg0 += idx_x * 1*1 + idx_y * 1*1 * dims_flux_calc_kernely[0][0];
  arg1 += idx_x * 1*1 + idx_y * 1*1 * dims_flux_calc_kernely[1][0];
  arg2 += idx_x * 1*1 + idx_y * 1*1 * dims_flux_calc_kernely[2][0];
...
void ops_par_loop_flux_calc_kernely_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_flux_calc_kernely_h[0][0] || xdim1 != dims_flux_calc_kernely_h[1][0] || xdim2 != dims_flux_calc_kernely_h[2][0] || xdim3 != dims_flux_calc_kernely_h[3][0]) {
    ...
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_flux_calc_kernely), dims_flux_calc_kernely_h, sizeof(dims_flux_calc_kernely)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    hipLaunchKernelGGL(ops_flux_calc_kernely, grid, tblock, 0, 0,
