...
//
__constant__ int dims_update_halo_kernel3_plus_2_b [3][1];
static int dims_update_halo_kernel3_plus_2_b_h [3][1] = {{0}};

//user function
__device__

inline void update_halo_kernel3_plus_2_b_gpu(ACC<double> &vol_flux_x,
...
int size1 ){
  ...
  if (size0==-1) dims_update_halo_kernel3_plus_2_b[0][0]=0;


  int idx_y = hipBlockDim_y * hipBlockIdx_y + hipThreadIdx_y;
  int idx_x = hipBlockDim_x * hipBlockIdx_x + hipThreadIdx_x;

  arg0 += idx_x * 1*1 + idx_y * 1*1 * dims_update_halo_kernel3_plus_2_b[0][0];
  arg1 += idx_x * 1*1 + idx_y * 1*1 * dims_update_halo_kernel3_plus_2_b[1][0];

  if (idx_x < size0 && idx_y < size1) {
    ACC<double> argp0(dims_update_halo_kernel3_plus_2_b[0][0], arg0);
    ACC<double> argp1(dims_update_halo_kernel3_plus_2_b[1][0], arg1);
    update_halo_kernel3_plus_2_b_gpu(
     argp0, argp1, arg2);
  }
...
void ops_par_loop_update_halo_kernel3_plus_2_b_execute(ops_kernel_descriptor *desc) {
  ...
  if (block->instance->OPS_diags > 1) {
    ops_timing_realloc(block->instance,36,"update_halo_kernel3_plus_2_b");
  ...
  if (xdim0 != dims_update_halo_kernel3_plus_2_b_h[0][0] || xdim1 != dims_update_halo_kernel3_plus_2_b_h[1][0]) {
    dims_update_halo_kernel3_plus_2_b_h[0][0] = xdim0;
    dims_update_halo_kernel3_plus_2_b_h[1][0] = xdim1;
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_update_halo_kernel3_plus_2_b), dims_update_halo_kernel3_plus_2_b_h, sizeof(dims_update_halo_kernel3_plus_2_b)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    hipLaunchKernelGGL(ops_update_halo_kernel3_plus_2_b, grid, tblock, 0, 0,
