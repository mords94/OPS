...
int size1 ){
  ...
  if (size0==-1) dims_reset_field_kernel1[0][0]=0;


  int idx_y = hipBlockDim_y * hipBlockIdx_y + hipThreadIdx_y;
  int idx_x = hipBlockDim_x * hipBlockIdx_x + hipThreadIdx_x;

  arg0 += idx_x * 1*1 + idx_y * 1*1 * dims_reset_field_kernel1[0][0];
  arg1 += idx_x * 1*1 + idx_y * 1*1 * dims_reset_field_kernel1[1][0];
  arg2 += idx_x * 1*1 + idx_y * 1*1 * dims_reset_field_kernel1[2][0];
...
void ops_par_loop_reset_field_kernel1_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_reset_field_kernel1_h[0][0] || xdim1 != dims_reset_field_kernel1_h[1][0] || xdim2 != dims_reset_field_kernel1_h[2][0] || xdim3 != dims_reset_field_kernel1_h[3][0]) {
    dims_reset_field_kernel1_h[0][0] = xdim0;
    dims_reset_field_kernel1_h[1][0] = xdim1;
    dims_reset_field_kernel1_h[2][0] = xdim2;
    dims_reset_field_kernel1_h[3][0] = xdim3;
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_reset_field_kernel1), dims_reset_field_kernel1_h, sizeof(dims_reset_field_kernel1)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    hipLaunchKernelGGL(ops_reset_field_kernel1, grid, tblock, 0, 0,
