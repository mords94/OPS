...
int size1 ){
  ...
  if (idx_x < size0 && idx_y < size1) {
    ...
    field_summary_kernel_gpu(
     argp0, argp1, argp2, argp3, argp4,
     argp5, arg6_l, arg7_l, arg8_l, arg9_l,
     arg10_l);
  }
  for (int d=0; d<1; d++)
    ops_reduction_hip<OPS_INC>(&arg6[d+(hipBlockIdx_x+ hipBlockIdx_y*hipGridDim_x)*1],arg6_l[d]);
  for (int d=0; d<1; d++)
    ops_reduction_hip<OPS_INC>(&arg7[d+(hipBlockIdx_x+ hipBlockIdx_y*hipGridDim_x)*1],arg7_l[d]);
  for (int d=0; d<1; d++)
    ops_reduction_hip<OPS_INC>(&arg8[d+(hipBlockIdx_x+ hipBlockIdx_y*hipGridDim_x)*1],arg8_l[d]);
  for (int d=0; d<1; d++)
    ops_reduction_hip<OPS_INC>(&arg9[d+(hipBlockIdx_x+ hipBlockIdx_y*hipGridDim_x)*1],arg9_l[d]);
  for (int d=0; d<1; d++)
    ops_reduction_hip<OPS_INC>(&arg10[d+(hipBlockIdx_x+ hipBlockIdx_y*hipGridDim_x)*1],arg10_l[d]);
...
void ops_par_loop_field_summary_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  for (int d=0; d<arg6.dim; d++) ((double *)arg6.data)[d+b*arg6.dim] = ZERO_double;
  reduct_bytes += ROUND_UP(maxblocks*arg6.dim*sizeof(double));

  arg7.data = block->instance->OPS_reduct_h + reduct_bytes;
  arg7.data_d = block->instance->OPS_reduct_d + reduct_bytes;
  for (int b=0; b<maxblocks; b++)
  for (int d=0; d<arg7.dim; d++) ((double *)arg7.data)[d+b*arg7.dim] = ZERO_double;
  reduct_bytes += ROUND_UP(maxblocks*arg7.dim*sizeof(double));

  arg8.data = block->instance->OPS_reduct_h + reduct_bytes;
  arg8.data_d = block->instance->OPS_reduct_d + reduct_bytes;
  for (int b=0; b<maxblocks; b++)
  for (int d=0; d<arg8.dim; d++) ((double *)arg8.data)[d+b*arg8.dim] = ZERO_double;
  reduct_bytes += ROUND_UP(maxblocks*arg8.dim*sizeof(double));

  arg9.data = block->instance->OPS_reduct_h + reduct_bytes;
  arg9.data_d = block->instance->OPS_reduct_d + reduct_bytes;
  for (int b=0; b<maxblocks; b++)
  for (int d=0; d<arg9.dim; d++) ((double *)arg9.data)[d+b*arg9.dim] = ZERO_double;
  reduct_bytes += ROUND_UP(maxblocks*arg9.dim*sizeof(double));

  arg10.data = block->instance->OPS_reduct_h + reduct_bytes;
  arg10.data_d = block->instance->OPS_reduct_d + reduct_bytes;
  for (int b=0; b<maxblocks; b++)
  for (int d=0; d<arg10.dim; d++) ((double *)arg10.data)[d+b*arg10.dim] = ZERO_double;
  reduct_bytes += ROUND_UP(maxblocks*arg10.dim*sizeof(double));
  ...
  if (x_size > 0 && y_size > 0)
    hipLaunchKernelGGL(ops_field_summary_kernel, grid, tblock, nshared, 0,
