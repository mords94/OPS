...
void initialise_chunk_kernel_y_gpu(ACC<double> &vertexy,
  const ACC<int> &yy,
  ACC<double> &vertexdy) {
...
int size1 ){
  ...
  if (size0==-1) dims_initialise_chunk_kernel_y[0][0]=0;


  int idx_y = hipBlockDim_y * hipBlockIdx_y + hipThreadIdx_y;
  int idx_x = hipBlockDim_x * hipBlockIdx_x + hipThreadIdx_x;

  arg0 += idx_x * 0*1 + idx_y * 1*1 * dims_initialise_chunk_kernel_y[0][0];
  arg1 += idx_x * 0*1 + idx_y * 1*1 * dims_initialise_chunk_kernel_y[1][0];
...
void ops_par_loop_initialise_chunk_kernel_y_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_initialise_chunk_kernel_y_h[0][0] || xdim1 != dims_initialise_chunk_kernel_y_h[1][0] || xdim2 != dims_initialise_chunk_kernel_y_h[2][0]) {
    dims_initialise_chunk_kernel_y_h[0][0] = xdim0;
    dims_initialise_chunk_kernel_y_h[1][0] = xdim1;
    dims_initialise_chunk_kernel_y_h[2][0] = xdim2;
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_initialise_chunk_kernel_y), dims_initialise_chunk_kernel_y_h, sizeof(dims_initialise_chunk_kernel_y)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    hipLaunchKernelGGL(ops_initialise_chunk_kernel_y, grid, tblock, 0, 0,
     (double *)p_a[0], (int *)p_a[1],
         (double *)p_a[2],
    x_size, y_size);
