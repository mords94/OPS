                                ...
                                const ACC<double> &celldx, const ACC<double> &vel1)
{
        ...
        limiter = wind * MIN(width * ((2.0 - sigma) * adw / width + (1.0 + sigma) * auw / celldx(1, 0)) / 6.0, MIN(auw, adw));
        limiter2 = wind2 * MIN(width * ((2.0 - sigma2) * adw / width + (1.0 + sigma2) * auw2 / celldx(-1, 0)) / 6.0, MIN(auw2, adw));
                                ...
                                const ACC<double> &celldy, const ACC<double> &vel1)
{
        ...
        limiter = wind * MIN(width * ((2.0 - sigma) * adw / width + (1.0 + sigma) * auw / celldy(0, 1)) / 6.0, MIN(auw, adw));
        limiter2 = wind2 * MIN(width * ((2.0 - sigma2) * adw / width + (1.0 + sigma2) * auw2 / celldy(0, -1)) / 6.0, MIN(auw2, adw));
                                ...
                                const ACC<double> &node_mass_pre, const ACC<double> &mom_flux)
{
        ...
        vel1(0, 0) = (vel1(0, 0) * node_mass_pre(0, 0) +
                      mom_flux(0, -1) - mom_flux(0, 0)) /
                     node_mass_post(0, 0);
}

inline void advec_mom_kernel_mass_flux_x_and_post_pre_advec_x(ACC<double> &node_mass_post,
                                                              ...
                                                              const ACC<double> &mass_flux_x)
{
        ...
        double node_flux_00 =  0.25 * (mass_flux_x(0, -1) + mass_flux_x(0, 0) +
                                  mass_flux_x(1, -1) + mass_flux_x(1, 0)); 

        //      node_flux(-1, 0) = 0.25 * (mass_flux_x(-1, -1) + mass_flux_x(-1, 0) +
        //                                 mass_flux_x(0, -1) + mass_flux_x(0, 0));
        double node_flux_M10 = 0.25 * (mass_flux_x(-1, -1) + mass_flux_x(-1, 0) +
                                  mass_flux_x(0, -1) + mass_flux_x(0, 0)); 
        ...
        node_mass_pre(0, 0) =
            node_mass_post(0, 0) - node_flux_M10 + node_flux_00;
}

inline void advec_mom_fusion(
    ACC<double> &vel1_temp,
