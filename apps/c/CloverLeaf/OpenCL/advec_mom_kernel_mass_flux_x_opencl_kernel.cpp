...
//
    ...
    void buildOpenCLKernels_advec_mom_kernel_mass_flux_x(OPS_instance *instance, int xdim0, int xdim1) {
      ...
      if(!isbuilt_advec_mom_kernel_mass_flux_x) {
        ...
        instance->ostream() <<"Compiling advec_mom_kernel_mass_flux_x "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_advec_mom_kernel_mass_flux_x=%d  -Dxdim1_advec_mom_kernel_mass_flux_x=%d ", pPath, 32,xdim0,xdim1);
            else
          ...
          instance->ostream() << "compiling advec_mom_kernel_mass_flux_x -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[78] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_advec_mom_kernel_mass_flux_x", &ret);
        clSafeCall( ret );
...
void ops_par_loop_advec_mom_kernel_mass_flux_x(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  if (block->instance->OPS_diags > 1) {
    ops_timing_realloc(block->instance,78,"advec_mom_kernel_mass_flux_x");
  ...
  buildOpenCLKernels_advec_mom_kernel_mass_flux_x(block->instance,
  xdim0,xdim1);
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[78], 2, sizeof(cl_int), (void*) &base0 ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[78], 3, sizeof(cl_int), (void*) &base1 ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[78], 4, sizeof(cl_int), (void*) &x_size ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[78], 5, sizeof(cl_int), (void*) &y_size ));
