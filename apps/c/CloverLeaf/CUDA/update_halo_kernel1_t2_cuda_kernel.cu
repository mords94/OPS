...
//
__constant__ int dims_update_halo_kernel1_t2 [8][1];
static int dims_update_halo_kernel1_t2_h [8][1] = {0};
...
int size1 ){
  ...
  arg6 += idx_x * 1*1 + idx_y * 1*1 * dims_update_halo_kernel1_t2[6][0];
  ...
  if (idx_x < size0 && idx_y < size1) {
    ...
    ACC<double> argp6(dims_update_halo_kernel1_t2[6][0], arg6);
    update_halo_kernel1_t2_gpu(
...
void ops_par_loop_update_halo_kernel1_t2_execute(ops_kernel_descriptor *desc) {
  ...
  if (!ops_checkpointing_before(args,8,range,11)) return;
  #endif

  if (block->instance->OPS_diags > 1) {
    ops_timing_realloc(block->instance,11,"update_halo_kernel1_t2");
  ...
  if (xdim0 != dims_update_halo_kernel1_t2_h[0][0] || xdim1 != dims_update_halo_kernel1_t2_h[1][0] || xdim2 != dims_update_halo_kernel1_t2_h[2][0] || xdim3 != dims_update_halo_kernel1_t2_h[3][0] || xdim4 != dims_update_halo_kernel1_t2_h[4][0] || xdim5 != dims_update_halo_kernel1_t2_h[5][0] || xdim6 != dims_update_halo_kernel1_t2_h[6][0]) {
    ...
    cutilSafeCall(block->instance->ostream(), cudaMemcpyToSymbol( dims_update_halo_kernel1_t2, dims_update_halo_kernel1_t2_h, sizeof(dims_update_halo_kernel1_t2)));
  }
