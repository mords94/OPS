...
int size1 ){
  ...
  if (idx_x < size0 && idx_y < size1) {
    ...
    initialise_chunk_kernel_xx_gpu(
     argp0, arg_idx);
  }
...
void ops_par_loop_initialise_chunk_kernel_xx_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_initialise_chunk_kernel_xx_h[0][0]) {
    ...
    cutilSafeCall(block->instance->ostream(), cudaMemcpyToSymbol( dims_initialise_chunk_kernel_xx, dims_initialise_chunk_kernel_xx_h, sizeof(dims_initialise_chunk_kernel_xx)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    ops_initialise_chunk_kernel_xx<<<grid, tblock >>> ( 
