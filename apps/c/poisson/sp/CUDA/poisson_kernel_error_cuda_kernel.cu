...
void poisson_kernel_error_gpu(const ACC<float> &u,
  const ACC<float> &ref,
  float *err) {
...
int size1 ){
  ...
  if (idx_x < size0 && idx_y < size1) {
    const ACC<float> argp0(dims_poisson_kernel_error[0][0], arg0);
    const ACC<float> argp1(dims_poisson_kernel_error[1][0], arg1);
...
void ops_par_loop_poisson_kernel_error_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_poisson_kernel_error_h[0][0] || xdim1 != dims_poisson_kernel_error_h[1][0]) {
    ...
    cutilSafeCall(block->instance->ostream(), cudaMemcpyToSymbol( dims_poisson_kernel_error, dims_poisson_kernel_error_h, sizeof(dims_poisson_kernel_error)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    ops_poisson_kernel_error<<<grid, tblock, nshared >>> (  (float *)p_a[0], (float *)p_a[1],
         (float *)arg2.data_d,x_size, y_size);
