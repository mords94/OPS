...
void poisson_kernel_error_c_wrapper(
  ...
  for ( int n_y=0; n_y<y_size; n_y++ ){
    for ( int n_x=0; n_x<x_size; n_x++ ){
      float err[1];
      err[0] = 0;
      const ptr_float u = { u_p + n_x*1 + n_y * xdim0_poisson_kernel_error*1, xdim0_poisson_kernel_error};
      const ptr_float ref = { ref_p + n_x*1 + n_y * xdim1_poisson_kernel_error*1, xdim1_poisson_kernel_error};
