...
//
    ...
    void buildOpenCLKernels_poisson_kernel_update(OPS_instance *instance, int xdim0, int xdim1) {
      ...
      if(!isbuilt_poisson_kernel_update) {
        ...
        instance->ostream() <<"Compiling poisson_kernel_update "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_poisson_kernel_update=%d  -Dxdim1_poisson_kernel_update=%d ", pPath, 32,xdim0,xdim1);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_poisson_kernel_update=%d  -Dxdim1_poisson_kernel_update=%d ", pPath, 32,xdim0,xdim1);
          else {
          ...
          instance->ostream() << "compiling poisson_kernel_update -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[1] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_poisson_kernel_update", &ret);
        clSafeCall( ret );
...
void ops_par_loop_poisson_kernel_update(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  buildOpenCLKernels_poisson_kernel_update(block->instance,
  xdim0,xdim1);
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[1], 5, sizeof(cl_int), (void*) &y_size ));
