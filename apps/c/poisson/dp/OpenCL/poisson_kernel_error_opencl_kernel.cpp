...
//
    ...
    void buildOpenCLKernels_poisson_kernel_error(OPS_instance *instance, int xdim0, int xdim1) {
      ...
      if(!isbuilt_poisson_kernel_error) {
        ...
        instance->ostream() <<"Compiling poisson_kernel_error "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_poisson_kernel_error=%d  -Dxdim1_poisson_kernel_error=%d ", pPath, 32,xdim0,xdim1);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_poisson_kernel_error=%d  -Dxdim1_poisson_kernel_error=%d ", pPath, 32,xdim0,xdim1);
          else {
          ...
          instance->ostream() << "compiling poisson_kernel_error -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[4] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_poisson_kernel_error", &ret);
        clSafeCall( ret );
...
void ops_par_loop_poisson_kernel_error(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2) {
  ...
  buildOpenCLKernels_poisson_kernel_error(block->instance,
  xdim0,xdim1);
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[4], 3, nthread*sizeof(double), NULL));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[4], 4, sizeof(cl_int), (void*) &r_bytes2 ));
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[4], 8, sizeof(cl_int), (void*) &y_size ));
